...
save_dir = "D:/Documents/research spring 24/Xact data/plots/colored"
...
# To ensure matching pairs of concentration and uncertainty columns
# assuming the pattern is "Element Number (ng/m3)" for concentration
# and "Element Uncert (ng/m3)" or "Element uncert (ng/m3)" for uncertainty,
# you might need a more sophisticated approach to handle inconsistencies in naming:
...
# Define a function to plot the time series for a given element
def plot_element_time_series(element_name):
    # Make sure the DataFrame is sorted by the index
    df_combined.sort_index(inplace=True)

    # Group by each day and plot
    fig, ax = plt.subplots(figsize=(12, 6))
    grouped = df_combined.groupby([df_combined.index.year, df_combined.index.month, df_combined.index.day])
    for (year, month, day), group in grouped:
        # Convert index to hour of the day
        group['hour'] = group.index.hour + group.index.minute / 60
        # Plot each day's line
        ax.plot(group['hour'], group[element_name], label=f'{year}-{month:02d}-{day:02d}')
    # Check for a detection limit and plot it
    ...
    # Add legend
    ax.legend(title='Date', loc='upper right', fontsize='x-small')

    # Set title and labels
    ax.set_title(f'Daily Time Series of {element_name} Concentration')
